...
const LoginForm = () => {
  ...
  return (
    <Container className="p-4 border border-secondary-subtle rounded-3 shadow">
      <h2 className="h2 mb-3">{t('loginPage.title')}</h2>
      <Formik
        initialValues={{
          username: '',
          password: '',
        ...
        {({ handleSubmit, handleChange, values, errors, submitCount, isSubmitting }) => (
          <Form onSubmit={handleSubmit}>
            ...
            <Button disabled={isSubmitting} type="submit" className="w-100 mb-3">
              {isSubmitting ? t('loginPage.loginProcess') : t('loginPage.login')}
            </Button>
            <div className="small">
              {t('loginPage.noAccount')} <a href="/signup">{t('loginPage.signup')}</a>
            </div>
          </Form>
        )}
      </Formik>
